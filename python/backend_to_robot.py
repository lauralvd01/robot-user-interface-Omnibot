...
async def goto():
    async with Channel(host='192.168.50.153', port=6550) as channel:
        robot = api.BasicRobotControlStub(channel)
        point = {"data": b'0', "format": PointFormat["PointFormat_XY_44"], "frame": "string"}
        try:
...
# BatteryDescriptor = {
#   BatteryState state = 1;     // State of health
#   float state_of_charge = 2;  // Unitless [0, 1]
#   float current = 3;  // Amps flowing in (+) or out (-) of the battery
#   float temperature = 4;  // Temperature in degrees Celcius
#   repeated uint32 cell_voltages = 5;   // Voltage of each cell in millivolts
# }
...
async def get_batteries_request():
    async with Channel(host='192.168.50.153', port=6550) as channel:
        ...
        try:
            ...
            print("Robot response", response)
            if response :
                for name,descriptor in response.batteries.items():
                    print(name)
                    print(descriptor)
                    print()
        except Exception as e :
